...
// Builtins returns the set of built-in specialist manifests.
func Builtins() []Manifest {
	builtins := []Manifest{
		{
			Metadata: Metadata{
				...
				Tools:       []string{"read-only", "edit", "execute"},
			},
	...
	for index := range builtins {
		if err := Validate(&builtins[index]); err != nil {
			panic("invalid built-in specialist: " + err.Error())
		}
	}
	return builtins
}

const workerPrompt = `You are a focused task specialist inside cli_mate.
...
const explorerPrompt = `You are a read-only codebase exploration specialist inside cli_mate.
...
const codeReviewPrompt = `You are a code review specialist inside cli_mate.
