...
func New(apiKey string, http *httpclient.Client) *Client {
	return &Client{apiKey: apiKey, baseURL: "https://api.x.ai/v1", http: http}
}
...
func (c *Client) StreamChat(ctx context.Context, req contracts.ChatRequest) (<-chan contracts.StreamEvent, error) {
	...
	if keyErr != nil {
		return nil, fmt.Errorf("decrypt xai api key: %w", keyErr)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("call xai: %w", err)
	}
...
func responseError(resp *http.Response) error {
	...
	if json.Unmarshal(body, &errResp) == nil && errResp.Error.Message != "" {
		return fmt.Errorf("xai error (%d): %s", resp.StatusCode, errResp.Error.Message)
	}
	return fmt.Errorf("xai returned status %d: %s", resp.StatusCode, truncate(string(body), 200))
}
