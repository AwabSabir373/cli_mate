...
func New(baseURL string, http *httpclient.Client) *Client {
	...
	return &Client{baseURL: strings.TrimRight(baseURL, "/"), http: http}
}
...
func (c *Client) StreamChat(ctx context.Context, req contracts.ChatRequest) (<-chan contracts.StreamEvent, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("encode lmstudio request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("call lmstudio: %w", err)
	}
...
func responseError(resp *http.Response) error {
	...
	return fmt.Errorf("lmstudio returned status %d: %s", resp.StatusCode, truncate(string(body), 200))
}
