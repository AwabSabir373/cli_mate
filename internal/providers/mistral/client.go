...
func New(apiKey string, http *httpclient.Client) *Client {
	return &Client{apiKey: apiKey, baseURL: "https://api.mistral.ai/v1", http: http}
}
...
func (c *Client) StreamChat(ctx context.Context, req contracts.ChatRequest) (<-chan contracts.StreamEvent, error) {
	...
	if keyErr != nil {
		return nil, fmt.Errorf("decrypt mistral api key: %w", keyErr)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("call mistral: %w", err)
	}
...
func responseError(resp *http.Response) error {
	...
	if json.Unmarshal(body, &errResp) == nil && errResp.Error.Message != "" {
		return fmt.Errorf("mistral error (%d): %s", resp.StatusCode, errResp.Error.Message)
	}
	return fmt.Errorf("mistral returned status %d: %s", resp.StatusCode, truncate(string(body), 200))
}
