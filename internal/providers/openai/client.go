...
func New(apiKey string, http *httpclient.Client) *Client {
	return &Client{apiKey: apiKey, baseURL: "https://api.openai.com/v1", http: http}
}
...
func (c *Client) StreamChat(ctx context.Context, req contracts.ChatRequest) (<-chan contracts.StreamEvent, error) {
	...
	if keyErr != nil {
		return nil, fmt.Errorf("decrypt openai api key: %w", keyErr)
	}
...
func responseError(resp *http.Response) error {
	...
	return fmt.Errorf("openai returned status %d: %s", resp.StatusCode, truncate(string(body), 200))
}
